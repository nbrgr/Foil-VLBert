...
class BatchCollator(object):
    ...
    def __call__(self, batch):
        ...
        for i, ibatch in enumerate(batch):
            ...
            pos = ibatch[self.data_names.index('pos')]
            out['pos'] = torch.as_tensor(pos).long()

            out['target'] = torch.as_tensor(ibatch[self.data_names.index('target')]).long()

            out['label'] = torch.as_tensor(ibatch[self.data_names.index('label')]).long()

            mask_len = ibatch[self.data_names.index('mask')]

            out['mask'] = torch.zeros_like(out['expression'])
            out['mask'][pos: pos + mask_len] = 1
