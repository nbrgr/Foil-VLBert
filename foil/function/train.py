...
from common.metrics import foil_metrics
...
from foil.data.build import make_dataloader, build_dataset, build_transforms
from foil.modules import *
from foil.function.val import do_validation
...
def train_net(args, config):
    ...
    if args.dist:
        ...
        if config.TRAIN.GRAD_ACCUMULATE_STEPS > 1:
            batch_size = batch_size * config.TRAIN.GRAD_ACCUMULATE_STEPS
        base_lr = config.TRAIN.LR * batch_size
        optimizer_grouped_parameters = [{'params': [p for n, p in model.named_parameters() if _k in n and p.requires_grad],
                                         'lr': base_lr * _lr_mult}
                                        for _k, _lr_mult in config.TRAIN.LR_MULT]
        optimizer_grouped_parameters.append({'params': [p for n, p in model.named_parameters()
                                                        if all([_k not in n for _k, _ in config.TRAIN.LR_MULT]) and p.requires_grad]})
    ...
    # metrics
    train_metrics_list = [foil_metrics.ClsAccuracy(allreduce=args.dist,
                                                      num_replicas=world_size if args.dist else 1),
                          foil_metrics.PosAccuracy(allreduce=args.dist,
                                                         num_replicas=world_size if args.dist else 1),
                          foil_metrics.CorAccuracy(allreduce=args.dist,
                                                         num_replicas=world_size if args.dist else 1)
                          ]
    val_metrics_list = [foil_metrics.ClsAccuracy(allreduce=args.dist,
                                                      num_replicas=world_size if args.dist else 1),
                          foil_metrics.PosAccuracy(allreduce=args.dist,
                                                         num_replicas=world_size if args.dist else 1),
                          foil_metrics.CorAccuracy(allreduce=args.dist,
    ...
    for output_name, display_name in config.TRAIN.LOSS_LOGGERS:
        train_metrics_list.append(
            foil_metrics.LossLogger(output_name, display_name=display_name, allreduce=args.dist,
                                       num_replicas=world_size if args.dist else 1))
